...
bool AlexRobot::initialiseJoints() {
    for (int id = 0; id < ALEX_NUM_JOINTS; id++) {
        ...
        std::cout << "[AlexRobot:initialiseJoints] Joint " << id << " is initialised at address " << joints << std::endl;
        std::cout << "[AlexRobot:initialiseJoints] Drive " << id + 1 << " is initialised at address " < motorDrives << std::endl;
