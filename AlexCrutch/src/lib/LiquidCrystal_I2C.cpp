...
#define printIIC(handle, args) i2c_write_byte(handle, args)

inline void LiquidCrystal_I2C::writeI2C(uint8_t value)
{
	send(value, Rs);
}
...
LiquidCrystal_I2C::LiquidCrystal_I2C(uint8_t lcd_Addr, uint8_t lcd_cols, uint8_t lcd_rows, uint8_t i2c_bus)
{
	_Addr = lcd_Addr;
	_cols = lcd_cols;
	_rows = lcd_rows;
...
int LiquidCrystal_I2C::i2c_open(unsigned char bus, unsigned char addr)
{
	int file;
	char filename[16];
...
int LiquidCrystal_I2C::i2c_close(int handle)
{
	if ((close(handle)) != 0)
	{
...
int LiquidCrystal_I2C::i2c_write_byte(int handle, unsigned char val)
{
	...
	else if (_commControl == 1)
	{
		...
		int i = 0;
		while (write(handle, &comm, 1) != 1 && i < 10)
		{
			i++;
		}
		if (i >= 10)
...
void LiquidCrystal_I2C::init_priv()
{
	...
	_handle = i2c_open(_i2c_bus, _Addr);
	begin(_cols, _rows);
}
...
void LiquidCrystal_I2C::clear()
{
	...
	if (_commControl)
	{
		for (auto i = 0; i < 2; i++)
		{
			commandQueue.push(SLEEP); // Pushes two skip/sleep commands to queue
		}
	}
	else
	{
...
void LiquidCrystal_I2C::writeStr(std::string str)
{
	int n = str.length();
	char char_array[n + 1];
...
int LiquidCrystal_I2C::isQueueEmpty()
{
...
int LiquidCrystal_I2C::getHandle()
{
	return this->_handle;
}
